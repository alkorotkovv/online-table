...
import {options, secondaryHeadCells} from "../../constants";
...
export default function EnhancedSecondaryTableBody() {

   console.log("EnhancedSecondaryTableBody")

  const dispatch = useDispatch()
  // const rowsState = useSelector(state => state.table.rows)
  const rowsState = useSelector(state => state.secondaryTable.rows)
  ...
  const columnsArray = secondaryHeadCells.slice(1);
      ...
      {/*{rowsState.map((row, index) => {*/}
        ...
        return (
          ...
          >
            ...
            <StickyTableCell align="center" sx={isItemSelected?{ left: secondaryHeadCells.slice(0, 0).reduce(function (sum, column) { return sum + column.width}, 54), backgroundColor: '#93abf3'} : {left: secondaryHeadCells.slice(0, 0).reduce(function (sum, column) { return sum + column.width}, 54)}}>
              <CustomTextAreaCell column={secondaryHeadCells[0].id} row={row.id} value={page * rowsPerPage + index+1} disabled={true}/>
            </StickyTableCell>
            {
              columnsArray.map((column, index) =>
                column.fixed ?
                  <StickyTableCell key={column.id}
                                   align="center"
                                   sx={{ left: secondaryHeadCells.slice(0, index+1).reduce((sum, column) => { return sum + column.width}, 54), backgroundColor: isItemSelected && '#93abf3'}}>
